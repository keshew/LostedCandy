...
extension BoomBlust {
    func getRandomItem() -> [String] {
        let allItems: [String] = [CandyImageName.bb1.rawValue,
                                  CandyImageName.bb2.rawValue,
                                  CandyImageName.bb3.rawValue,
                                  CandyImageName.bb4.rawValue,
                                  CandyImageName.bb5.rawValue,
                                  CandyImageName.bb6.rawValue]
        return allItems
    }
    
    func setupView() {
        let background = SKSpriteNode(imageNamed: CandyImageName.gameBackH.rawValue)
        background.size = CGSize(width: size.width, height: size.height)
        background.position = CGPoint(x: size.width / 2, y: size.height / 2)
        addChild(background)
        
        let backGameLabel = SKSpriteNode(imageNamed: CandyImageName.backLabelBoomBlust.rawValue)
        ...
        let playDesk = SKSpriteNode(imageNamed: CandyImageName.playDesk.rawValue)
        ...
        let menuBack = SKSpriteNode(imageNamed: CandyImageName.menuBack.rawValue)
        ...
        candyLabel.text = "\(String(describing: game!.numberOfCandy))"
        ...
        timeLabel = SKLabelNode(fontNamed: "LuckiestGuy-Regular")
        timeLabel.text = "0:30"
        timeLabel.fontSize = 40
        timeLabel.position = CGPoint(x: size.width / 3, y: size.height / 1.72)
    ...
    func addNewElement(atRow row: Int, col: Int) {
        ...
        if randomItem == CandyImageName.bb1.rawValue {
            element.name = "bb1"
        }
    ...
    func initializeGame() {
        ...
        boundaryNode = SKSpriteNode(color: .clear, size: CGSize(width: 170, height: 20))
